...
public function toggleStatus(Request $request, $id_etudiant)
{
    ...
    public function manageAcademics()
    {
       ...
       $departements = Departement::all();
        ...
        $semestres   = Semestres::orderBy('id_semestre')->get();
        $pivot = DB::table('matiere_filieres')->get();

        return view('superadmin.academics', compact('departements', 'filieres', 'semestres','matieres','pivot'));
    ...
    public function getEtudiantsParMatiere(Request $request)
{
    try {
       ...
       // ✅ Fallback ultime — tous les étudiants de la filière via inscriptions
if ($etudiants->isEmpty()) {
    $etudiants = Etudiants::with('filiere.departement')
        ->whereHas('inscriptions', function($q) use ($matiere) {
            $q->where('id_filiere', $matiere->id_filiere);
        })
        ->orderBy('nom')->get();

    // Si vraiment rien, on prend tous les étudiants sans filtre
    if ($etudiants->isEmpty()) {
        $etudiants = Etudiants::with('filiere.departement')
            ->orderBy('nom')->get();
    }
...
$qrCode = QrCode::format('svg')
    ...
    return response()->json([
        ...
        'html' => view(
            'superadmin.etudiants.releve-contenu',
...
public function relevePdf($id_etudiant)
{
    $etudiant = Etudiants::with(['filiere.departement'])
        ->findOrFail($id_etudiant);
    ...
    $moyenneGenerale = $totalCreditsTableau > 0
        ? round($totalPoints / $totalCreditsTableau, 2)
        : 0;
    ...
    $sessionActive = 'Normale';

    $signataire = 'Dr. Landry AZOGAN';
...
// ── Route web.php ──────────────────────────────────────────────
// Route::get('/super-admin/notes/pdf/releves/{id}', [SuperAdminController::class, 'telechargersReleve'])
//      ->name('superadmin.pdf.releves');
// ──────────────────────────────────────────────────────────────

public function telechargersReleve(Request $request, $id)
    {
        $etudiant = Etudiants::with([
        ...
        $anneeAcademique = $request->get('annee',       '2025-2026');
        $sessionActive   = $request->get('session',     'Normale');
        $signataire      = $request->get('signataire',  'Le Directeur');
        $download        = $request->boolean('download', false);

        $notesQuery = $etudiant->notes()->with('matiere');

        if (!empty($semestre)) {
            $notesQuery->whereHas('matiere', fn($q) => $q->where('semestre', $semestre));
        }

        $notesQuery->where('session', strtolower($sessionActive));
        $notes = $notesQuery->get();

        $totalCreditsTableau = $notes->sum(fn($n) => $n->matiere->credit_matiere ?? 1);
        $creditsValides = $notes->filter(fn($n) => $n->valeur_note >= 10)->sum(fn($n) => $n->matiere->credit_matiere ?? 1);
        ...
        // Code document unique généré en amont
        $codeDocument = 'UAC-' . strtoupper(uniqid());
        $urlVerification = url('/verify-document/' . $codeDocument);
        ...
        $viewData = [
            ...
            'totalCreditsTableau' => $totalCreditsTableau,
        ...
        if ($request->ajax()) {
            $html = view('superadmin.etudiants.releve-contenu', $viewData)->render();
            return response()->json(['html' => $html]);
        }

        // Configuration stricte de DomPDF pour éviter les attentes réseau infinies
        $pdf = Pdf::loadView('superadmin.etudiants.releve-contenue', $viewData)
            ...
            ->setOptions([
                ...
                'dpi'                  => 150,
                'chroot'               => public_path(), // Bloque la recherche d'images en dehors du dossier public
            ]);

        $filename = 'releve_'
            . strtolower(str_replace(' ', '_', $etudiant->nom))
            . '_' . str_replace('-', '_', $anneeAcademique)
            . '.pdf';
